...
export default (props) => {
  ...
  const globalState = {
    ...
    list: useSelector((state) => state.photo.list),
    references: useSelector((state) => state.photo.references),
    count: useSelector((state) => state.photo.count),
    rowCount: useSelector((state) => state.photo.pagingRowCount),
  ...
  const localActions = {
    ...
    load: () =>
      dispatch(actions.photo.load({ page, sortKey, sortOrder, filterString })),
  ...
  let tableColumns = [
    {
      ...
      render: (tmp, row) => (
        <span>
          <Link to={`/photo/detail/${row.id}`}>{row.id}</Link>
        </span>
      ),
    },
              ...
              {row.photo.map((file) => {
                return (
                  <li key={file.localFilename}>
                    <a
                      href={`${conf.baseRouteURL}/api/photo/file/${row.id}/${file.localFilename}`}
                    >
                      {file.thumbs && file.thumbs["128"] ? (
                        <img
                          class="small"
                          src={`${conf.baseRouteURL}/api/photo/file/${row.id}/${file.thumbs["128"]}`}
                        />
                      ) : (
...

    {
      ...
      render: (tmp, row) => (
        <span>
          <Link to={`/tag/detail/${tagIdList[row.tag]}`}>
            {tagList[row.tag]}
          </Link>
        </span>
      ),
    ...
    },
    {
      ...
      render: (tmp, row) => (
        <span>
          <Link to={`/photo/edit/${row.id}`}>Edit </Link>
          &nbsp;<Link to={`/photo/delete/${row.id}`}>Delete</Link>
        </span>
      ),
    },
  ];
  ...
  return (
    ...
    >
      <Button
        ...
        icon="plus"
        shape="circle"
        onClick={(e) => history.push("/photo/add")}
      />
